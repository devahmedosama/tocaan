...
@section('content')
	<div class="page-header">
        <h2 class="header-title"> {{ trans('home.Units') }} </h2>
        <div class="header-sub-title">
            <nav class="breadcrumb breadcrumb-dash">
                <a href="{{ URL::to('admin') }}" class="breadcrumb-item"><i class="anticon anticon-home m-r-5"></i>{{ trans('home.Home') }}</a>
                <span class="breadcrumb-item active">{{ trans('home.Units') }}</span>
            </nav>
        </div>
    </div>
	<div class="card">
		<div class="card-body">
			<a href="{{ URL::to('admin/units/add') }}" class="btn btn-md btn-primary pull-right">{{ trans('home.Add Unit') }}</a>
			    ...
			    <table class="table">
			        ...
			        <tbody>
			            ...
			            <tr>
			                <td>{{ $data->name }}</td>
			                <td>
			                	<a href="{{ URL::to('admin/units/edit/'.$data->id) }}" class="btn btn-primary btn-xs">
								  ...
								  <div class="modal-dialog" role="document">
								    <div class="modal-content">
								      ...
								      <div class="modal-footer">
								        ...
								        <a href="{{ URL::to('admin/units/delete/'.$data->id) }}" class="btn btn-danger">{{ trans('home.Confirm') }}</a>
								      </div>
								    </div>
								  </div>
								</div>
