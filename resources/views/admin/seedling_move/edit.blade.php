...
@section('content')
	...
	<div class="card">
		<div class="card-body">
			{{ Form::open(['url'=>'admin/seedling-moves/edit/'.$data->id,'files'=>true,'enctype'=>'multipart']) }}
			    
			    <div class="form-row">
			    	
					<div class="form-group col-md-6">
			            <label for="inputEmail4">{{ trans('home.Nursery') }}</label>
			            {{ Form::select('nursery_id',$nurseries,$data->nursery_id,['class'=>'form-control basic-select','required']) }}
			        </div>
					<div class="form-group col-md-6">
			            <label for="inputEmail4">{{ trans('home.Farm') }}</label>
			            {{ Form::select('farm_id',$farms,$data->farm_id,['class'=>'form-control basic-select','required']) }}
			        </div>
			    	<div class="form-group col-md-6">
			            <label for="inputEmail4">{{ trans('home.Seedling No') }}</label>
			            {{ Form::number('seedling_no',$data->seedling_no,['class'=>'form-control ','min'=>1,'required']) }}
			        </div>
			        <div class="form-group col-md-6">
