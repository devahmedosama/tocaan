...
@section('content')
	<div class="page-header">
        <h2 class="header-title"> {{ trans('home.Suppliers') }} </h2>
        <div class="header-sub-title">
            <nav class="breadcrumb breadcrumb-dash">
                <a href="{{ URL::to('admin') }}" class="breadcrumb-item"><i class="anticon anticon-home m-r-5"></i>{{ trans('home.Home') }}</a>
                <span class="breadcrumb-item active">{{ trans('home.Suppliers') }}</span>
            </nav>
        </div>
    </div>
	<div class="card">
		<div class="card-body">
			<a href="{{ URL::to('admin/suppliers/add') }}" class="btn btn-md btn-primary pull-right">{{ trans('home.Add Supplier') }}</a>
			    ...
			    <table class="table">
			        ...
			        <tbody>
			            ...
			            <tr>
			                <td>{{ $data->name }}</td>
			                <td>
			                	<a href="{{ URL::to('admin/suppliers/edit/'.$data->id) }}" class="btn btn-primary btn-xs">
								  ...
								  <div class="modal-dialog" role="document">
								    <div class="modal-content">
								      ...
								      <div class="modal-footer">
								        ...
								        <a href="{{ URL::to('admin/suppliers/delete/'.$data->id) }}" class="btn btn-danger">{{ trans('home.Confirm') }}</a>
								      </div>
								    </div>
								  </div>
								</div>
