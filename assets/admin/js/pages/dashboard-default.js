...
$(".activate_user").click(function () {
    var id = $(this).data("id");
    let el = $(this);
    $.get(base_url + "/admin/users/activate/" + id, function (data) {
...
$(".feature_user").click(function () {
    var id = $(this).data("id");
    let el = $(this);
    $.get(base_url+ "/admin/users/feature/"+id, function (data) {
        let state = data.state;
        if (state == 1) {
            el.html("Remove From Feature");
            el.addClass("active");
            el.removeClass("stop");
        } else {
            el.html("Add To Feature");
            el.removeClass("active");
            el.addClass("stop");
        }
    });
...
$('.delete_item').click(function(){
    var id =  $(this).data('id');
    var el = $(this);
    $.get(base_url+ "/admin/users/rate/"+id, function (data) {
        let state = data.state;
        el.parent().remove();
    });
})
